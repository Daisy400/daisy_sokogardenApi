...
def signup():
    ...
    connection=pymysql.connect(host="localhost",user="root",password="",database="daisy_sokogarden")
...
def signin():
   ...
   sql="select user_id, username, phone from users where email =%s and password=%s"
   data=(email,password)
   cursor.execute(sql,data)
...
def addproduct():
    ...
    # save image folder into images folder
    ...
    sql='''
        insert into product_details(product_name, product_description, product_category, product_cost, product_image)
        values(%s,%s,%s,%s,%s)
